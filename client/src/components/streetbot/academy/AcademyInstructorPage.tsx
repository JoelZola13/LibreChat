...
function getInstructorTabFromHash(hash: string): InstructorTab | null {
  const normalizedHash = hash.replace(/^#/, '');
  if (normalizedHash === 'instructor-certificate-generator') return 'certificate-generator';
  if (normalizedHash === 'instructor-learning-path-generator') return 'learning-path-generator';
  if (normalizedHash === 'instructor-add-course') return 'add-course';
  if (normalizedHash === 'instructor-generate') return 'generate';
  if (normalizedHash === 'instructor-courses') return 'courses';
  if (normalizedHash === 'instructor-schedule' || normalizedHash === 'instructor')
    return 'schedule';
  return null;
}
...
type AcademyInstructorPageProps = {
  ...
  activeTabOverride?: InstructorTab;
  onActiveTabChange?: (tab: InstructorTab) => void;
};
...
export default function AcademyInstructorPage({
  ...
  const routeActiveTab = getInstructorTab(location.pathname);
  const [embeddedActiveTab, setEmbeddedActiveTab] = useState<InstructorTab>(
    () => getInstructorTabFromHash(location.hash) || 'schedule',
  );
  const activeTab = embedded ? (activeTabOverride ?? embeddedActiveTab) : routeActiveTab;
  const usesDashboardRoute = embedded || location.pathname.includes('/dashboard/instructor');
  ...
  useEffect(() => {
    if (!embedded) return;
    const hashTab = getInstructorTabFromHash(location.hash);
    if (hashTab) {
      setEmbeddedActiveTab(hashTab);
      onActiveTabChange?.(hashTab);
    }
  }, [embedded, location.hash, onActiveTabChange]);
  ...
  const loadWorkspaceData = useCallback(async () => {
    ...
    try {
      ...
      setCohorts(
        (Array.isArray(cohortData) ? cohortData : []).filter((cohort: Cohort) =>
          instructorCourseIds.has(cohort.course_id),
        ),
      );
      setSessions(
        liveSessions.filter(
          (session) => instructorCourseIds.has(session.course_id) && session.status !== 'cancelled',
        ),
      );
  ...
  const handleGenerate = async (e: FormEvent) => {
    ...
    try {
      const requirementTags = splitTextareaLines(genRequirements).map(
      ...
      const existingCourse = editingCourseId
        ? (visibleAllCourses.find((course) => course.id === editingCourseId) ?? null)
        : null;
      ...
      const payload = {
        ...
        state: 'published',
        instructor_name:
          genInstructor || existingCourse?.instructor_name || 'Street Voices Academy',
      ...
      const resp = await sbFetch(
        editingCourseId ? `/api/academy/courses/${editingCourseId}` : '/api/academy/courses',
        {
      ...
      if (!resp.ok) {
        throw new Error(
          `Failed to ${editingCourseId ? 'update' : 'create'} course: ${resp.status}`,
        );
      }
  ...
  const handleDeleteCourse = async (course: Course) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Unable to delete this course right now.';
  ...
  const handleCreateLearningPath = async (event: FormEvent) => {
    ...
    const selectedCourses = availablePathCourses.filter((course) =>
      selectedPathCourseIds.includes(course.id),
    );
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Unable to create this program right now.';
  ...
  const handleDeleteLearningPath = async (pathSlug: string) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Unable to delete this program right now.';
      setPathResult(`Error: ${message}`);
    } finally {
  ...
  const certificatePathTargets = useMemo(() => visibleLearningPaths, [visibleLearningPaths]);
    ...
    void (async () => {
      try {
        const response = await sbFetch(
          `/api/academy/enrollments?course_id=${encodeURIComponent(certificateTargetId)}`,
        );
  ...
  const handleIssueCertificate = async (event: FormEvent) => {
    ...
    try {
      ...
      setCertificateResult(
        `Certificate sent to ${recipientName}. It is now available in the student's dashboard certificates section.`,
      );
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Unable to issue this certificate right now.';
  ...
  const topTabs: Array<{ tab: InstructorTab; label: string; href: string; icon: LucideIcon }> = [
    {
      tab: 'schedule',
      label: 'Schedule',
      href: embedded ? '#instructor-schedule' : `${basePath}/schedule`,
      icon: CalendarDays,
    },
    {
      tab: 'generate',
      label: 'Generate Course',
      href: embedded ? '#instructor-generate' : `${basePath}/generate`,
      icon: Sparkles,
    },
    {
      tab: 'certificate-generator',
      label: 'Certificate Generator',
      href: embedded ? '#instructor-certificate-generator' : `${basePath}/certificate-generator`,
      icon: Award,
    },
    {
      ...
      href: embedded
        ? '#instructor-learning-path-generator'
        : `${basePath}/learning-path-generator`,
      icon: Target,
    },
    {
      tab: 'courses',
      label: 'Courses',
      href: embedded ? '#instructor-courses' : `${basePath}/courses`,
      icon: BookOpen,
    },
    {
      tab: 'add-course',
    ...
    <div className="space-y-5">
      ...
      >
        ...
        <p className="mt-2 text-sm" style={{ color: colors.textSecondary }}>
          A calendar-style view of your live sessions, cohorts, and course updates across every
          class you teach.
        </p>
      </div>
  ...
  const renderCoursesTab = () => (
    <div className="space-y-5">
      ...
      >
        ...
        <p className="mt-2 text-sm" style={{ color: colors.textSecondary }}>
          Open any course to manage its teaching schedule, live sessions, grading, and learning
          materials.
        </p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visibleInstructorCourses.map((course) => {
            ...
            return (
              ...
              >
                <div
                  className="relative mb-5 overflow-hidden rounded-[22px] border"
                ...
                <p className="mt-3 text-sm" style={{ color: colors.textSecondary }}>
                  {course.description ||
                    'This course is ready to be managed inside the instructor teaching view.'}
                </p>
                <div
    ...
    <div className="space-y-6" style={{ maxWidth: 1120, margin: '0 auto' }}>
      ...
      >
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: colors.textSecondary }}>
              Create a full course in minutes. Enter a topic and generate structured lessons,
              modules, and outcomes to get started quickly.
            </p>
          </div>
        </div>
        ...
        {editingCourseId && (
          ...
          >
            You are editing an existing Academy course. Save your changes below or cancel to return
            to generator mode.
          </div>
        )}
        ...
        {genResult && (
          <div
            ...
            style={{
              background: genResult.startsWith('Error')
                ? 'rgba(239,68,68,0.12)'
                : 'rgba(34,197,94,0.12)',
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          ...
          <div>
            ...
            <select
              value={genDelivery}
              onChange={(e) => setGenDelivery(e.target.value)}
              style={inputStyle(colors, isDark)}
            >
      ...
      >
        ...
        {workspaceLoading ? (
          <div
            className="mt-5 rounded-[22px] border p-5 text-sm"
            style={{
              borderColor: colors.border,
              background: colors.cardBgStrong,
              color: colors.textSecondary,
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {visibleAllCourses.map((course) => {
              ...
              return (
                ...
                >
                  <div
                    className="relative mb-5 overflow-hidden rounded-[20px] border"
                    style={{ borderColor: colors.border }}
                  >
                    <img
                      src={visual.src}
                  ...
                  <p className="mt-3 text-sm" style={{ color: colors.textSecondary }}>
                    {course.description ||
                      'Academy course ready to be updated from the instructor workspace.'}
                  </p>
                  ...
                  >
                    ...
                    <span>
                      {course.instructor_name || course.instructor || 'Street Voices Academy'}
                    </span>
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <button
                      type="button"
                      onClick={() =>
                        isEditing ? handleCancelEditingCourse() : handleEditCourse(course)
                      }
      ...
      >
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: colors.textSecondary }}>
              Build a guided Academy plan by naming the path and choosing the courses it should
              include.
            </p>
          </div>
        </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {availablePathCourses.map((course) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="relative mb-4 overflow-hidden rounded-[18px] border"
                      style={{ borderColor: colors.border }}
                    >
                      <img
                        src={visual.src}
                    ...
                    <p className="mt-2 text-sm" style={{ color: colors.textSecondary }}>
                      {course.description ||
                        'This course can be added into a guided Academy program.'}
                    </p>
                    <div
                    ...
                    >
                      <span>{course.duration || 'Flexible duration'}</span>
                      <span>
                        {course.state === 'published' || !course.state ? 'Published' : course.state}
                      </span>
                    </div>
                  </button>
                );
      ...
      >
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {generatedPaths.length > 0 ? (
            ...
            <div
              className="rounded-[22px] border p-5 text-sm md:col-span-2 xl:col-span-3"
              style={{
                borderColor: colors.border,
                background: colors.cardBgStrong,
                color: colors.textSecondary,
            ...
            >
              No generated programs yet. Create one above to make it available on the learner
              program pages.
            </div>
          )}
        </div>
      ...
      >
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: colors.textSecondary }}>
              AI-powered certificate builder for issuing polished Academy certificates to the right
              students.
            </p>
          </div>
        </div>
        ...
        {certificateResult && (
          <div
            className="mb-5 rounded-[18px] border px-4 py-3 text-sm"
            style={{
              borderColor: certificateResult.startsWith('Error')
                ? 'rgba(239,68,68,0.3)'
                : colors.border,
              background: certificateResult.startsWith('Error')
                ? 'rgba(239,68,68,0.08)'
                : colors.cardBg,
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setCertificateAwardType(event.target.value as AwardTargetType)
                  }
                  style={inputStyle(colors, isDark)}
                >
              ...
              <div>
                ...
                >
                  <option value="">
                    {certificateAwardType === 'course' ? 'Select a course' : 'Select a program'}
                  </option>
                  {certificateAwardType === 'course'
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {eligibleLearners.map((learner) => (
                      <button
                        ...
                        style={{
                          background:
                            certificateRecipientId === learner.user_id
          ...
          >
            <div className="text-center">
              ...
              <p
                ...
                style={{ color: '#1F2937', fontFamily: 'Georgia, "Times New Roman", serif' }}
              >
                {certificateFullName.trim() || 'Full Name'}
              </p>
              <div
                className="mx-auto mt-4 h-px w-40"
                style={{ background: 'rgba(180,83,9,0.26)' }}
              />
              <p className="mx-auto mt-8 max-w-xl text-base leading-8" style={{ color: '#374151' }}>
                For successfully completing this{' '}
                {certificateAwardType === 'course' ? 'course' : 'program'} of{' '}
                <span style={{ fontWeight: 700, color: '#111827' }}>
                  {selectedCertificateTargetTitle ||
                    (certificateAwardType === 'course' ? 'Course Name' : 'Program Name')}
                </span>
              </p>
            </div>
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-2">
              <div
                ...
                style={{ borderColor: 'rgba(180,83,9,0.16)', background: 'rgba(255,255,255,0.7)' }}
              >
  ...
  const renderAddCourseTab = () => (
    <div className="space-y-5">
      ...
      >
        ...
        <p className="mt-2 text-sm" style={{ color: colors.textSecondary }}>
          Browse all Academy courses and add the ones you want to teach into your instructor
          workspace.
        </p>
      </div>
      ...
      {addCourseMessage && (
        <div
          ...
          style={{
            borderColor: addCourseMessage.startsWith('Error')
              ? 'rgba(239,68,68,0.3)'
              : colors.border,
            background: addCourseMessage.startsWith('Error')
              ? 'rgba(239,68,68,0.08)'
              : colors.cardBg,
          ...
          {visibleAllCourses.map((course) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-3 text-sm" style={{ color: colors.textSecondary }}>
                  {course.description ||
                    'Academy course ready to be added to an instructor workspace.'}
                </p>
                ...
                <div className="mt-5">
                  <button
                    ...
                    onClick={() =>
                      void (isMine ? handleUndoClaimCourse(course) : handleClaimCourse(course))
                    }
...
function inputStyle(colors: { border: string; text: string }, isDark: boolean): CSSProperties {
  return {
...
function buildLearningPathPayload(title: string, courses: Course[], instructorId: string) {
  ...
  return {
    ...
    what_youll_learn: courseTitles.map(
      (courseTitle) => `Build practical confidence through ${courseTitle}.`,
    ),
    milestones: courseTitles.map(
      (courseTitle, index) => `Step ${index + 1}: Complete ${courseTitle}.`,
    ),
    outcomes: [
