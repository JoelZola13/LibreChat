/**
 * Academy Video Captions API adapter for LibreChat (Vite).
 * Re-exports types and provides sbFetch-based API functions.
 */

import { sbFetch } from '../../shared/sbFetch';

const BASE = '/api/academy/captions';
...
export async function getLessonCaptions(lessonId: string): Promise<Caption[]> {
  try {
    const response = await sbFetch(`${BASE}/lessons/${lessonId}`);
...
export async function uploadCaption(
  ...
  const response = await sbFetch(
    `${BASE}/lessons/${lessonId}/upload?${params.toString()}`,
    {
      method: 'POST',
      body: formData,
    }
...
export async function updateCaption(
  ...
  const response = await sbFetch(`${BASE}/${captionId}`, {
    method: 'PUT',
...
export async function setDefaultCaption(captionId: string): Promise<void> {
  const response = await sbFetch(`${BASE}/${captionId}/set-default`, {
    method: 'POST',
  });
  if (!response.ok) throw new Error('Failed to set default caption');
}
...
export async function getSupportedLanguages(): Promise<SupportedLanguage[]> {
  try {
    const response = await sbFetch(`${BASE}/languages`);
    if (!response.ok) return [];
    return response.json();
  } catch {
    return [];
