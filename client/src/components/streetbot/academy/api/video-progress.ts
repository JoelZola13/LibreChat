/**
 * Academy Video Progress API adapter for LibreChat (Vite).
 * Re-exports types and provides sbFetch-based API functions.
 */

import { sbFetch } from '../../shared/sbFetch';

const BASE = '/api/academy/video';
...
export async function getVideoProgress(lessonId: string, userId: string): Promise<VideoProgress | null> {
  try {
    const response = await sbFetch(`${BASE}/progress/${lessonId}?user_id=${encodeURIComponent(userId)}`);
...
export async function updateVideoProgress(
  ...
  const response = await sbFetch(`${BASE}/progress/${lessonId}?${params}`, {
    method: 'POST',
...
export async function getLessonVideoStats(lessonId: string): Promise<LessonVideoStats> {
  const response = await sbFetch(`${BASE}/stats/${lessonId}`);
...
export async function createBookmark(
  ...
  const response = await sbFetch(`${BASE}/bookmarks/${lessonId}?user_id=${encodeURIComponent(userId)}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  });
...
export async function deleteBookmark(bookmarkId: string): Promise<void> {
  const response = await sbFetch(`${BASE}/bookmarks/${bookmarkId}`, {
    method: 'DELETE',
  });
...
export async function createNote(
  ...
  const response = await sbFetch(`${BASE}/notes/${lessonId}?user_id=${encodeURIComponent(userId)}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  });
...
export async function updateNote(noteId: string, content: string): Promise<VideoNote> {
  const response = await sbFetch(`${BASE}/notes/${noteId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
