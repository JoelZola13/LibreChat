 ...
 * Used by the /myprofile route so "Street Profile" dropdown shows the new page.
 */
...
import { ensureStreetProfileForUser } from './academyProfileSync';
import { findAcademyStreetProfileByUserId } from './academyStreetProfiles';

const SETTINGS_STORAGE_KEY = 'streetbot:user-settings';
const DEFAULT_MY_PROFILE_AVATAR =
  'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=420&h=420&fit=crop&auto=format';
const DEFAULT_MY_PROFILE_BANNER =
  'https://images.unsplash.com/photo-1517935706615-2717063c2225?w=1800&h=460&fit=crop&auto=format';
  ...
  const profile = useMemo(() => {
    ...
    return {
      ...
      username:
        ...
        'street_user',
      display_name:
        syncedProfile?.display_name ||
        user?.name ||
        academyProfile?.display_name ||
        sp.name ||
        'Street User',
      primary_roles: sp.title
        ? [sp.title]
        : syncedProfile?.primary_roles || academyProfile?.primary_roles || ['Creative'],
      ...
      avatar_url:
        ...
        DEFAULT_MY_PROFILE_AVATAR,
      cover_url:
        sp.bannerUrl ||
        syncedProfile?.cover_url ||
        academyProfile?.cover_url ||
        DEFAULT_MY_PROFILE_BANNER,
      city:
        sp.location?.split(',')[0]?.trim() ||
        syncedProfile?.city ||
        academyProfile?.city ||
        'Toronto',
      country:
        sp.location?.split(',')[1]?.trim() ||
        syncedProfile?.country ||
        academyProfile?.country ||
        'ON',
      location_display:
        sp.location ||
        syncedProfile?.location_display ||
        academyProfile?.location_display ||
        'Toronto, ON',
      portfolio_items: [],
      external_links: [],
      website: sp.website || syncedProfile?.website || academyProfile?.website || null,
      availability_status:
        syncedProfile?.availability_status || academyProfile?.availability_status || 'open',
      ...
      completeness_score:
        syncedProfile?.completeness_score || academyProfile?.completeness_score || 40,
      created_at:
        syncedProfile?.created_at ||
        academyProfile?.created_at ||
        user?.createdAt ||
