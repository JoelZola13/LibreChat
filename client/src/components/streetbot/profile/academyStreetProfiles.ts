...
const GENERATED_ACADEMY_INSTRUCTOR_PROFILES: StreetProfileRecord[] =
  GENERATED_ACADEMY_INSTRUCTOR_NAMES.map((name, index) => {
    ...
    return {
      ...
      tagline:
        'Supporting Street Voices Academy learners through practical teaching and facilitation.',
      avatar_url: null,
      cover_url: null,
      city: 'Toronto',
      country: 'Canada',
...
const CASE_MANAGEMENT_STREET_PROFILES: StreetProfileRecord[] = CASE_MANAGEMENT_PROFILE_SEEDS.map(
  (seed, index) => {
...
const JOB_APPLICANT_STREET_PROFILES: StreetProfileRecord[] = [
  {
    id: 'job-applicant-profile-aaliyah-morgan',
    user_id: 'job-applicant-aaliyah-morgan',
    username: 'aaliyah-morgan',
    display_name: 'Aaliyah Morgan',
    primary_roles: ['Community Outreach Coordinator', 'Event Host'],
    secondary_skills: ['Neighbourhood engagement', 'Volunteer coordination', 'Story collection'],
    bio: 'Aaliyah designs outreach moments for community programs, pop-up events, and story circles, with a focus on making first contact feel welcoming.',
    tagline: 'Building friendly bridges between programs, artists, and neighbourhood residents.',
    avatar_url: null,
    cover_url: null,
    city: 'Toronto',
    country: 'Canada',
    ...
    website: null,
    availability_status: 'open',
    open_to: ['Community outreach', 'Event support', 'Program coordination'],
    contact_email: null,
    contact_preference: 'form',
    is_public: true,
    is_featured: true,
  ...
  {
    id: 'job-applicant-profile-marcus-lee',
    user_id: 'job-applicant-marcus-lee',
    username: 'marcus-lee',
    display_name: 'Marcus Lee',
    primary_roles: ['Youth Program Assistant', 'Workshop Support'],
    secondary_skills: ['Mentorship', 'Facilitation', 'Attendance tracking'],
    bio: 'Marcus supports youth programming with calm facilitation, session prep, and one-on-one check-ins that help participants stay connected.',
    tagline: 'Helping youth programs feel organized, consistent, and human.',
    avatar_url: null,
    cover_url: null,
    city: 'Toronto',
    country: 'Canada',
    ...
    website: null,
    availability_status: 'open',
    open_to: ['Youth programs', 'Workshop assistance', 'Mentorship'],
    contact_email: null,
    contact_preference: 'form',
    is_public: true,
    is_featured: false,
  ...
  {
    id: 'job-applicant-profile-sofia-alvarez',
    user_id: 'job-applicant-sofia-alvarez',
    username: 'sofia-alvarez',
    display_name: 'Sofia Alvarez',
    primary_roles: ['Community Outreach Coordinator', 'Bilingual Organizer'],
    secondary_skills: ['Spanish outreach', 'Resource navigation', 'Intake support'],
    bio: 'Sofia connects residents with practical resources, helps teams organize outreach lists, and brings bilingual support to community-facing events.',
    tagline: 'Bilingual outreach rooted in care, clarity, and follow-through.',
    avatar_url: null,
    cover_url: null,
    city: 'Toronto',
    country: 'Canada',
    ...
    website: null,
    availability_status: 'open',
    open_to: ['Outreach coordination', 'Bilingual support', 'Community events'],
    contact_email: null,
    contact_preference: 'form',
    is_public: true,
    is_featured: true,
  ...
  {
    id: 'job-applicant-profile-daniel-kim',
    user_id: 'job-applicant-daniel-kim',
    username: 'daniel-kim',
    display_name: 'Daniel Kim',
    primary_roles: ['Grant Writer', 'Research Assistant'],
    secondary_skills: ['Proposal writing', 'Budget notes', 'Impact summaries'],
    bio: 'Daniel turns program details into clear grant language, concise impact stories, and funder-ready summaries for small community teams.',
    tagline: 'Writing practical grant copy that keeps community work moving.',
    avatar_url: null,
    cover_url: null,
    city: 'Toronto',
    country: 'Canada',
    ...
    website: null,
    availability_status: 'open',
    open_to: ['Grant writing', 'Research', 'Impact reporting'],
    contact_email: null,
    contact_preference: 'form',
    is_public: true,
    is_featured: false,
  ...
  {
    id: 'job-applicant-profile-jasmine-patel',
    user_id: 'job-applicant-jasmine-patel',
    username: 'jasmine-patel',
    display_name: 'Jasmine Patel',
    primary_roles: ['Youth Program Assistant', 'Peer Mentor'],
    secondary_skills: ['Peer support', 'Creative activities', 'Program logistics'],
    bio: 'Jasmine helps youth programs run smoothly by preparing activities, welcoming participants, and keeping session logistics tidy.',
    tagline: 'Creative youth support with steady hands and warm energy.',
    avatar_url: null,
    cover_url: null,
    city: 'Toronto',
    country: 'Canada',
    ...
    website: null,
    availability_status: 'open',
    open_to: ['Youth facilitation', 'Peer mentorship', 'Program support'],
    contact_email: null,
    contact_preference: 'form',
    is_public: true,
    is_featured: false,
...
function getAllAcademyStreetProfiles() {
  return [
    ...ACADEMY_STREET_PROFILES,
    ...GENERATED_ACADEMY_INSTRUCTOR_PROFILES,
    ...CASE_MANAGEMENT_STREET_PROFILES,
    ...JOB_APPLICANT_STREET_PROFILES,
  ];
}
...
export function findAcademyStreetProfileByUsername(
  username?: string | null,
): StreetProfileRecord | null {
  ...
  return (
    getAllAcademyStreetProfiles().find(
      (profile) => profile.username.toLowerCase() === normalized,
    ) ?? null
  );
}

export function findAcademyStreetProfileByUserId(
  userId?: string | null,
): StreetProfileRecord | null {
...
export function findAcademyStreetProfileByInstructorName(
  instructorName?: string | null,
): StreetProfileRecord | null {
  const normalized = String(instructorName || '')
    .trim()
    .toLowerCase();
  if (!normalized) {
    return null;
  ...
  return (
    getAllAcademyStreetProfiles().find((profile) => {
      ...
      const academyInstructorName = String(profile.academy_instructor_name || '')
        .trim()
        .toLowerCase();
...
export function getAcademyRoleForProfile(
  profile?: {
    user_id?: string | null;
    username?: string | null;
    primary_roles?: string[];
  ...
  if (
    normalizedRoles.some(
      (role) =>
        role.includes('instructor') || role.includes('facilitator') || role.includes('teacher'),
    )
  ) {
    return 'instructor';
  }
...
export function getInstructorNameForProfile(
  profile?: {
    user_id?: string | null;
    username?: string | null;
...
function inferFallbackRole(
  profile: StreetProfileHydrationInput,
  seeded?: StreetProfileRecord | null,
): AcademyProfileRole {
  if (profile.academy_role) {
    return profile.academy_role;
  }
...
export function hydrateStreetProfileRecord(
  profile: StreetProfileHydrationInput,
): StreetProfileRecord {
  const seeded =
    findAcademyStreetProfileByUserId(profile.user_id) ??
  ...
  return {
    ...
    availability_status: String(
      profile.availability_status ?? seeded?.availability_status ?? 'open',
    ),
    ...
    academy_instructor_name:
      profile.academy_instructor_name ?? seeded?.academy_instructor_name ?? null,
  };
}

export function mergeStreetProfiles<T extends { username: string }>(
  primary: T[],
  secondary: T[],
): T[] {
