...
import { parseStoredJson, readStoredBoolean } from '../shared/safeStorage';
...
function readUserLocation(): UserLocation | null {
  ...
  try {
    ...
    const parsed = parseStoredJson<Record<string, unknown> | null>(stored, null);
...
const SERVICES_API_URL = DIRECTORY_SERVICES_API_URL;
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: compactMobile ? '12px' : isMobileCard ? '14px' : '20px' }}>
        ...
        >
          <Link
            ...
            onMouseEnter={(e) => {
              ...
              prefetchServiceDetail(service.id);
            }}
            onMouseLeave={(e) => {
        ...
        {isMobileCard ? (
          <div
            style={{ marginBottom: '10px', display: 'flex', flexDirection: 'column', gap: '4px' }}
          >
            <div
              style={{
                display: 'flex',
            ...
            >
              <MapPin size={13} color={DIRECTORY_CONTACT_ACCENT} style={{ flexShrink: 0 }} />
            ...
            {service.phone && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px' }}>
                <Phone size={13} color={DIRECTORY_CONTACT_ACCENT} style={{ flexShrink: 0 }} />
                <a
          ...
          >
            ...
            {service.phone && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <Phone size={16} color={DIRECTORY_CONTACT_ACCENT} style={{ flexShrink: 0 }} />
                <a
                  href={`tel:${service.phone.replace(/\D/g, '')}`}
                  onClick={(e) => e.stopPropagation()}
                  style={{
          ...
          >
            ...
            >
              ...
              >
                ...
                {service.phone && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <Phone size={15} color={DIRECTORY_CONTACT_ACCENT} style={{ flexShrink: 0 }} />
                    <a
                      href={`tel:${service.phone.replace(/\D/g, '')}`}
                      style={{
                        fontSize: '13px',
...
export default function DirectoryPage() {
  ...
  const effectiveProgressiveLimit = isMapActive
    ? DIRECTORY_MAP_BATCH
    : getDirectorySearchBatchSize();
  ...
  const searchResult = useDirectorySearch({
    ...
    radiusKm:
      nearMeActive && userLocation && filters.maxDistance > 0 ? filters.maxDistance : undefined,
  ...
  const transformService = (svc: Record<string, unknown>): Service => {
    ...
    const structuredPhone =
      (contact?.phone_secondary as string) || (contact?.phone as string) || undefined;
    ...
    return {
      ...
      longitude: lng,
      logo:
        (svc.image_url as string) || (svc.logo_url as string) || (svc.logo as string) || undefined,
      distance,
  ...
  useEffect(() => {
    setSidebarMinimized(readStoredBoolean('sidebarMinimized', true));
  }, []);
  const navLeft =
  ...
  const colors = useMemo(
    () => ({
      ...baseColors,
      accentText: dark ? baseColors.accent : '#111827',
    }),
    [baseColors, dark],
  );
  ...
  const services = useMemo(() => {
    return searchResult.hits.map((hit) =>
      transformService(hit as unknown as Record<string, unknown>),
    );
  }, [searchResult.hits]);
  ...
  const dynamicCategoryOptions = useMemo(() => {
    ...
    const result = CATEGORY_OPTIONS.filter((opt) => facetNames.has(opt.value)).map((opt) => ({
      value: opt.value,
      label: opt.label,
      icon: opt.icon,
    }));
  ...
  const visibleServices = isMapActive
    ? filteredServices
  ...
  const mapServices = useMemo(() => {
    ...
    if (filters.maxDistance > 0 && userLocation) {
      result = result.filter((s) => {
        ...
        const dist = calculateDistance(userLocation.lat, userLocation.lon, s.latitude, s.longitude);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const resp = await fetch(`${SERVICES_API_URL}?limit=5000&skip=0`, { signal });
        if (resp.ok) {
      ...
      const SEARCH_ENDPOINTS = [DIRECTORY_SEARCH_API_URL, `${SB_API_BASE}/services/search`];
      ...
      for (const ep of SEARCH_ENDPOINTS) {
        ...
        try {
          const testResp = await fetch(`${ep}?q=&page=0&per_page=1`, { signal });
          if (testResp.ok) {
  ...
  const toggleFavorite = useCallback(
    async (id: number) => {
      ...
      try {
        const resp = await fetch(`${base}/${id}/favorite?user_id=${encodeURIComponent(userId)}`, {
  ...
  const mobileSearchInputPadding = isUltraNarrowMobile ? 8 : isNarrowMobile ? 10 : 14;
  ...
  return (
    ...
    >
        ...
        {isLargeDesktop ? (
          <nav
            style={{
              ...
              order: 1,
              gap: 4,
              marginLeft: 'max(0px, calc(50vw - 649.5px))',
              flexShrink: 0,
              whiteSpace: 'nowrap',
            }}
          >
      ...
      <div
        style={{
          margin: '0 auto',
          padding: isMobile ? '16px 12px' : isTablet ? '16px 18px' : '16px 24px',
          paddingTop: isMobile ? 87 : isTablet ? 78 : 86,
          position: 'relative',
          zIndex: 1,
        }}
      >
        ...
        >
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: isUltraNarrowMobile
                ? 'flex-start'
                : isMobile
                  ? 'space-between'
                  : 'flex-start',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {isResultsLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                <Building2 size={48} color={colors.textMuted} style={{ marginBottom: '16px' }} />
                <h3
                  style={{
                    fontSize: '1.125rem',
                    fontWeight: 600,
                ...
                {showMapList && (
                  <div style={{ maxHeight: '300px', overflowY: 'auto', padding: '12px' }}>
                    {mapServices.map((service) => (
                      ...
                      >
                        <MapPin size={14} color={DIRECTORY_CONTACT_ACCENT} />
                        <span style={{ fontSize: '14px', fontWeight: 500, color: colors.text }}>
                          {service.name}
                        </span>
                        <span
                          style={{ fontSize: '12px', color: colors.textMuted, marginLeft: 'auto' }}
                        >
                          {service.distance
                        ...
                        <div style={{ flex: 1, minWidth: 0, width: '100%' }}>
                          ...
                          >
                            <div>
                              ...
                              >
                                ...
                                {service.is_verified && <BadgeCheck size={18} color="#3b82f6" />}
                              </div>
                              <div
                                style={{
                                  display: 'inline-flex',
                            ...
                            >
                              ...
                              >
                                <Heart
                                  size={20}
                                  color={favorites.has(service.id) ? '#ef4444' : colors.textMuted}
                                  fill={favorites.has(service.id) ? '#ef4444' : 'none'}
                                />
                              </button>
                              <button
                                onClick={(e) => {
                          ...
                          >
                            ...
                            >
                              <MapPin size={14} color={DIRECTORY_CONTACT_ACCENT} />
                              <span
                                style={{ fontSize: '13px', color: colors.text, lineHeight: 1.45 }}
                              >
                                {service.address || service.city || 'Location not available'}
                              </span>
                            </div>
                            {service.phone && (
                              ...
                              >
                                <Phone size={14} color={DIRECTORY_CONTACT_ACCENT} />
                                <a
                                  href={`tel:${service.phone!.replace(/\D/g, '')}`}
                                  onClick={(e) => e.stopPropagation()}
                                  style={{
                      ...
                      >
                        ...
                        {userLocation ? (
                          ...
                          <p style={{ fontSize: '11px', color: colors.textMuted, margin: 0 }}>
                            Use button below to set location
                          </p>
                        )}
                      </div>
                      <button
