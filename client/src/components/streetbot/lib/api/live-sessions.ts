/**
 * Academy Live Sessions API Integration
 * Webinars, workshops, live classes, Q&A, polls, and attendance tracking
 */

const API_BASE = '/sbapi';
const LIVE_SESSIONS_API = `${API_BASE}/academy/live-sessions`;
...
export async function createSession(data: CreateSessionData, instructorId: string): Promise<LiveSession> {
  const response = await fetch(`${LIVE_SESSIONS_API}?instructor_id=${instructorId}`, {
    method: "POST",
...
export async function getSession(sessionId: string, userId?: string): Promise<SessionWithRegistration> {
  ...
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}?${params}`);
...
export async function getCourseSessions(
  ...
  const response = await fetch(`${LIVE_SESSIONS_API}/course/${courseId}?${params}`);
  if (!response.ok) throw new Error("Failed to fetch sessions");
  return response.json();
}

export async function updateSession(sessionId: string, data: UpdateSessionData): Promise<LiveSession> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
...
export async function startSession(sessionId: string): Promise<LiveSession> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/start`, { method: "POST" });
  if (!response.ok) throw new Error("Failed to start session");
  return response.json();
}

export async function endSession(sessionId: string): Promise<LiveSession> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/end`, { method: "POST" });
...
export async function cancelSession(sessionId: string, reason?: string): Promise<void> {
  ...
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/cancel?${params}`, { method: "POST" });
...
export async function registerForSession(sessionId: string, userId: string): Promise<SessionRegistration> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/register?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
  });
  if (!response.ok) throw new Error("Failed to register for session");
  return response.json();
}
...
export async function getSessionRegistrations(sessionId: string): Promise<SessionRegistration[]> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/registrations`);
...
export async function getUserUpcomingSessions(userId: string): Promise<{ sessions: Array<{ registration: SessionRegistration; session: LiveSession }>; total: number }> {
  const response = await fetch(`${LIVE_SESSIONS_API}/user/${encodeURIComponent(userId)}/upcoming`);
...
export async function getUserAllSessions(userId: string): Promise<{ sessions: Array<{ registration: SessionRegistration; session: LiveSession }>; total: number }> {
  const response = await fetch(`${LIVE_SESSIONS_API}/user/${encodeURIComponent(userId)}/all`);
  if (!response.ok) throw new Error("Failed to fetch user sessions");
  return response.json();
}
...
export async function joinSession(sessionId: string, userId: string): Promise<{ status: string; meeting_url: string | null; registration: SessionRegistration }> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/join?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
  });
...
export async function leaveSession(sessionId: string, userId: string): Promise<{ status: string; attendance_duration: number }> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/leave?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
  });
...
export async function createPoll(sessionId: string, data: CreatePollData): Promise<SessionPoll> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/polls`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
...
export async function startPoll(pollId: string): Promise<SessionPoll> {
  const response = await fetch(`${LIVE_SESSIONS_API}/polls/${pollId}/start`, { method: "POST" });
  if (!response.ok) throw new Error("Failed to start poll");
  return response.json();
}

export async function endPoll(pollId: string): Promise<SessionPoll> {
  const response = await fetch(`${LIVE_SESSIONS_API}/polls/${pollId}/end`, { method: "POST" });
  if (!response.ok) throw new Error("Failed to end poll");
  return response.json();
}

export async function submitPollResponse(pollId: string, userId: string, response: unknown): Promise<void> {
  const res = await fetch(`${LIVE_SESSIONS_API}/polls/${pollId}/respond?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getPollResults(pollId: string): Promise<PollResults> {
  const response = await fetch(`${LIVE_SESSIONS_API}/polls/${pollId}/results`);
...
export async function askQuestion(
  ...
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/questions?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function getSessionQuestions(sessionId: string, status?: string): Promise<SessionQuestion[]> {
  ...
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/questions?${params}`);
  if (!response.ok) throw new Error("Failed to fetch questions");
  return response.json();
}

export async function upvoteQuestion(questionId: string, userId: string): Promise<void> {
  const response = await fetch(`${LIVE_SESSIONS_API}/questions/${questionId}/upvote?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
  });
...
export async function answerQuestion(questionId: string, answer: string, answeredBy: string): Promise<SessionQuestion> {
  const response = await fetch(`${LIVE_SESSIONS_API}/questions/${questionId}/answer?answered_by=${encodeURIComponent(answeredBy)}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function submitFeedback(sessionId: string, userId: string, data: SessionFeedbackData): Promise<void> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/feedback?user_id=${encodeURIComponent(userId)}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
...
export async function getFeedbackSummary(sessionId: string): Promise<FeedbackSummary> {
  const response = await fetch(`${LIVE_SESSIONS_API}/${sessionId}/feedback/summary`);
...
export function getSessionStatus(session: LiveSession): {
  label: string;
  color: string;
  canJoin: boolean;
  isUpcoming: boolean;
  ...
  if (session.status === "cancelled") {
    return { label: "Cancelled", color: "red", canJoin: false, isUpcoming: false, isLive: false, isPast: true };
  }

  if (session.status === "live") {
    return { label: "Live Now", color: "green", canJoin: true, isUpcoming: false, isLive: true, isPast: false };
  }

  if (session.status === "ended" || now > end) {
    return { label: "Ended", color: "gray", canJoin: false, isUpcoming: false, isLive: false, isPast: true };
  }

  // Scheduled
  const minutesUntilStart = (start.getTime() - now.getTime()) / (1000 * 60);
  if (minutesUntilStart <= 15) {
    return { label: "Starting Soon", color: "yellow", canJoin: false, isUpcoming: true, isLive: false, isPast: false };
  }
