...
- Selected draft-lane receipts now show a readable wiki draft preview before publication. The preview renders a Wikipedia-style title, lead paragraphs, contents, article sections, citation markers, backlinks, and a not-promoted safety stamp so reviewed batches visibly become wiki-shaped pages before promotion, embedding, Neo4j sync, attachment, cleanup, move, or delete gates open.
- Selected draft-lane receipts now also create a human promotion packet. The packet gathers readiness gates, blocker reasons, a source manifest, article markdown, references, backlinks, and locked-write reminders so a selected batch can be reviewed as a publishable wiki article candidate before any article promotion, embedding, Neo4j sync, attachment, cleanup, move, or delete action is allowed.
- Selected draft-lane receipts now include a promotion review desk. It makes the human publish decision, proposed article page id, citation rule, locked memory writes, first blocker, metadata workup refresh, and article review target explicit while keeping every action navigation-only or metadata-only until a separate publish, embedding, Neo4j, attachment, cleanup, move, or delete gate is opened.
